...
class EventRepository
  ...
  def store(aggregate, event_class, expected_version: :any)
    ...
    Rails.logger.debug("[RES] Storing to stream=#{stream} unpublished_events=#{aggregate.unpublished_events.map(&:class).join(', ')}")
    ...
    Rails.logger.debug("[RES] Publishing event #{event.class.name} with expected_version=#{expected_version}")
    @client.publish(
      event,
      stream_name: stream,
    ...
    Rails.logger.debug("[RES] Stored #{aggregate.unpublished_events.count} events to #{stream}")
  end
